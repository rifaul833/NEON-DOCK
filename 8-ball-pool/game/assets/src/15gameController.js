...
*/
			...
			if(gameInfo.mode != "practice"){
				if (gameInfo.fouled == true) {
					...
					if(!gameInfo.fouled) {
						...
						if(gameInfo.turn == "p1") {
							famobi_analytics.trackEvent("EVENT_LIVESCORE", { liveScore: projectInfo.score });
						}
					}
	...
	function showGameOver(forceWinP1 = false){
		...
		if(projectInfo.mode == 2){
			...
			if(gameInfo.winner == "p1"){
				...
				gameInfo.p2Icon.scale = new Phaser.Point(.4, .4);

				gameInfo.p2Icon.y = pIconY + (256 * .05);
				gameInfo.p1Icon.y = pIconY;
			}else{
				gameInfo.p1Icon.rosette.visible = false;
				gameInfo.p1Icon.scale = new Phaser.Point(.4, .4);

				gameInfo.p2Icon.y = pIconY;
				gameInfo.p1Icon.y = pIconY + (256 * .05);
			}
	...
	window.FORCE_LOSE = (forceP1 = true) => { showGameOver(forceP1); }
